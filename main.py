...
def mainwindow():
    ...
    sidebar = ttk.Frame(window, width=45, height=300, borderwidth=0.3,relief='raised')
    sidebar.pack(side='left', fill='both')

    sidebar2= ttk.Frame(window, height=45, relief='sunken')
    sidebar2.pack(side='top', fill='both')

    # search

    search = StringVar()
    searchentry = ttk.Entry(sidebar2, textvariable=search,width=40)
    searchentry.place(relx=0.37, rely=0.07)
    searchicon = PhotoImage(file=r"images\search.png")
    searchimage = searchicon.subsample(25, 25)
    searchbutton = ttk.Button(sidebar2, image=searchimage)
    searchbutton.place(relx=0.57, rely=0.05)
    searchentry.bind("<MouseWheel>", lambda event: my_canvas.yview_scroll(-1*(event.delta//120), "units"))
    searchbutton.bind("<MouseWheel>", lambda event: my_canvas.yview_scroll(-1*(event.delta//120), "units"))
    ...
    browser.place(relx=0.08, rely=0.01)
    browser.bind("<MouseWheel>", lambda event: my_canvas.yview_scroll(-1*(event.delta//120), "units"))
    ...
    communication.place(relx=0.08, rely=0.07)
    communication.bind("<MouseWheel>", lambda event: my_canvas.yview_scroll(-1*(event.delta//120), "units"))
    ...
    development.place(relx=0.08, rely=0.13)
    development.bind("<MouseWheel>", lambda event: my_canvas.yview_scroll(-1*(event.delta//120), "units"))
    ...
    utilities.place(relx=0.08, rely=0.19)
    utilities.bind("<MouseWheel>", lambda event: my_canvas.yview_scroll(-1*(event.delta//120), "units"))
    ...
    games.place(relx=0.08, rely=0.25)
    games.bind("<MouseWheel>", lambda event: my_canvas.yview_scroll(-1*(event.delta//120), "units"))
    ...
    multimedia.place(relx=0.08, rely=0.31)
    multimedia.bind("<MouseWheel>", lambda event: my_canvas.yview_scroll(-1*(event.delta//120), "units"))
    
    documenticon = PhotoImage(file=r"images\documents.png")
    documenticon = documenticon.subsample(18, 18)
    document = ttk.Button(sidebar, image= documenticon, padding=0, command=lambda: redirectpsn("documents"))
    document.place(relx=0.08, rely=0.37)
    document.bind("<MouseWheel>", lambda event: my_canvas.yview_scroll(-1*(event.delta//120), "units"))
    ...
    status = ttk.Label(title_bar, text="v.0.3.Alpha")
    status.place(relx=0.48, rely=0.4)

    theme = ttk.Button(title_bar, image=on, padding=0,command=lambda: switch())
    theme.place(relx=0.755, rely=0.1)
    if mode == "light":
        theme.config(image=off)
    ...
    def chocoinstall(appname):
        install_cmd = f"choco install {appname} -y"
        subprocess.run(install_cmd, shell=True)

    def chocouninstall(appname):
        uninstall_cmd = f"choco uninstall {appname} -y"
        subprocess.run(uninstall_cmd, shell=True)

    def chocoupdate(appname):
        upgrade_cmd = f"choco upgrade {appname} -y"
        subprocess.run(upgrade_cmd, shell=True)

    def scoopcheck():
        install_cmd = "iex (new-object net.webclient).downloadstring('https://get.scoop.sh')"
        subprocess.run(install_cmd, shell=True)

    def scoopinstall(appname):
        install_cmd = f"scoop install {appname}"
        subprocess.run(install_cmd, shell=True)

    def scoopuninsall(appname):
        uninstall_cmd = f"scoop uninstall {appname}"
        subprocess.run(uninstall_cmd, shell=True)

    def scoopupdate(appname):
        update_cmd = f"scoop update {appname}"
    ...
    def addapps(type):
        ...
        canvas.bind("<MouseWheel>", lambda event: my_canvas.yview_scroll(-1*(event.delta//120), "units"))
        ...
        frame3 = Frame(canvas)
        canvas.create_window((25, 15), window=frame3, anchor="nw")
        ...
        section = ttk.Label(sectionframe, text=type, font=("Segou UI variable", 22))
        ...
        if i / 2 < len(browserlists):
            ...
            button.bind("<Button-1>", lambda event, buttonimg=browserimgs[i // 2], buttonname=button.cget('text'),
                                             ...
                                             pckg=browserpacknames[i // 2]: [remold(),hoverwindow(event, buttonimg, buttonname,
                                                                                        buttondesc, pckg)])
            button.bind("<MouseWheel>", lambda event: my_canvas.yview_scroll(-1*(event.delta//120), "units"))
    # communication
    ...
    # Documents
    sectionframe = addapps("Documents")
    for i in range(1, 16, 2):
        spacing = ttk.Label(sectionframe, text="    ")
        spacing.grid(row=0, column=i)
        spacing.bind("<MouseWheel>", lambda event: my_canvas.yview_scroll(-1*(event.delta//120), "units"))
